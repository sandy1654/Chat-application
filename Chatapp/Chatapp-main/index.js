...
io.on('connection', socket => {
  ...
  socket.on('check_user', (asker, id) => {
    const username = Object.keys(usernames).find(key => usernames[key] === id);
    io.to(usernames[asker]).emit('msg_user_found', username);
  });
  ...
  socket.on('msg_user', (to_user, from_user, msg) => {
    ...
    // append the chat message to a txt file
    fs.appendFile('chat_data.txt', msg + '\r\n', (err) => {
      if (err) throw err;
    });
  });
});
